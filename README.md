...
3. Grab the jar file.
4. Run `java -jar simple-db-app-1.0-SNAPSHOT.jar -help` for help on available commands.
...
- It is currently necessary to pass the id of the server (e.g. `-i 123`), and there is currently no auto-increment.
- There is currently no test coverage.
- Currently you cannot pass -D switches on command line when calling the jar. This means environment variables must be set in system.